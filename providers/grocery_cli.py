...
STORES = {
    "mercadona": {"label":"Mercadona","key":"mercadona"},
    "gadis": {"label":"Gadis","key":"gadis"},
    "dia": {"label":"DIA","key":"dia"},
    "lidl": {"label":"Lidl","key":"lidl-es"},
...
CATEGORY_RULES = {
    ...
    "detergente": {
        "must_any": {"detergente"},
        "forbid": {"lavavajillas","limpiador","jabon","champu","gel"},
    ...
    "papel_higienico": {
        ...
        "forbid": {"cocina","horno","aluminio","regalo","envolver","fotografico"},
...
class GroceryCLI:
    ...
    def search(self,store,query,limit=8,ean=None):
        ...
        search_term=query
        data=self._run(["--store",cfg["key"],"search",search_term,"--limit","40","--json"])
        ...
        for p in products:
            ...
            p["normalized_price"]=normalized_price(p["price"],fu)
            p["rank_score"]=round(rank,3)

            eq=units_needed_for_equivalence(query,p["name"])
            p["equivalent_units"]=eq
            if eq:
                p["equivalent_cost"]=round(float(p["price"])*int(eq["units_needed"]),2)
            else:
        ...
        valid.sort(key=lambda p:(
            not p.get("exact_ean_match",False),
            0 if p.get("confidence")=="alta" else 1,
            -p.get("rank_score",0),
            p["normalized_price"] is None,
            p["normalized_price"] if p["normalized_price"] is not None else p["price"]
        ))
    ...
    def compare(self,stores,items):
        ...
        for store in stores:
            ...
            for item in items:
                ...
                line=best["price"]*units_needed*qty
                total+=line
                detail.append({
                    "query":q,
                    "qty":qty,
        ...
        return {"results":rows,"best":rows[0] if rows else None,"data_mode":"REAL_BETA"}
